...
@Component({
    ...
    template: `
        <span [ngClass]="{'ui-calendar':true, 'ui-calendar-w-btn': showIcon, 'ui-calendar-timeonly': timeOnly}"
              [ngStyle]="style"
              [class]="styleClass" style="width: 100%">
            <ng-template [ngIf]="!inline">
                <input #inputfield type="text" [attr.id]="inputId" [attr.name]="name" [attr.required]="required"
                       ...
                       (keydown)="onInputKeydown($event)" (click)="onInputClick()" (blur)="onInputBlur($event)"
                       ...
                       [ngClass]="'p-inputtext ui-widget ui-state-default ui-corner-all'" autocomplete="off"
            ...
            <div #contentWrapper [class]="panelStyleClass" [ngStyle]="panelStyle" [ngClass]="{'ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all': true, 'ui-datepicker-inline':inline,'ui-shadow':!inline,
                ...
                <ng-container *ngIf="!timeOnly">
                    <div class="ui-datepicker-group ui-widget-content" *ngFor="let month of months; let i = index;">
                        <div class="ui-datepicker-header ui-widget-header ui-helper-clearfix ui-corner-all">
                            <a class="ui-datepicker-prev ui-corner-all" (click)="onPrevButtonClick($event)"
                               (keydown.enter)="onPrevButtonClick($event)" *ngIf="i === 0" tabindex="0"
                               (keydown)="onInputKeydown($event)">
                                <span class="ui-datepicker-prev-icon">
                                    >
                                </span>
                            </a>
                            ...
                            <a class="ui-datepicker-next ui-corner-all" (click)="onNextButtonClick($event)"
                               (keydown.enter)="onNextButtonClick($event)"
                               *ngIf="numberOfMonths === 1 ? true : (i === numberOfMonths -1)"
                               tabindex="0" (keydown)="onInputKeydown($event)">
                                <span class="ui-datepicker-next-icon">
                                    <
                                </span>
                            </a>
                        </div>
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col"
                                            *ngFor="let weekDay of weekDays;let begin = first; let end = last">
                                            <span  [class]="end ? 'text-danger':'' ">{{weekDay}}</span>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr *ngFor="let week of month.dates; let j = index;">
                                        ...
                                        <td *ngFor="let date of week ;let begin = first; let end = last" [ngClass]="{'ui-datepicker-other-month': date.otherMonth,
                                            ...
                                            <ng-container *ngIf="date.otherMonth ? showOtherMonths : true">
                                                <a class="ui-state-default" *ngIf="date.selectable"
                                                   ...
                                                   (keydown)="onDateCellKeydown($event,date,i)">
                                                    <span *ngIf="!dateTemplate" [class]="end ? 'text-danger':'' ">
                                                ...
                                                <span class="ui-state-default ui-state-disabled"
                                                    ...
                                                    <ng-container
                                                            *ngIf="!disabledDateTemplate">{{date.day}}</ng-container>
                                                    <ng-container
                ...
                <div class="ui-timepicker ui-widget-header ui-corner-all" *ngIf="showTime||timeOnly">
                    <div class="ui-hour-picker">
                        <a tabindex="0" (keydown)="onContainerButtonKeydown($event)"
                ...
                <div class="ui-datepicker-buttonbar ui-widget-header" *ngIf="showButtonBar">
                    <button type="button" tabindex="0" [label]="_locale.today"
                            (keydown)="onContainerButtonKeydown($event)"
                            (click)="onTodayButtonClick($event)" pButton [ngClass]="[todayButtonStyleClass]"></button>
                    <button type="button" tabindex="0" [label]="_locale.clear"
                            (keydown)="onContainerButtonKeydown($event)"
                            (click)="onClearButtonClick($event)" pButton [ngClass]="[clearButtonStyleClass]"></button>
                </div>
...
export class PersianCalendarComponent implements OnInit, OnDestroy, ControlValueAccessor {
        ...
        for (let i = 0; i < monthRows; i++) {
            ...
            if (i == 0) {
                ...
                for (let j = 0; j < remainingDaysLength; j++) {
                    week.push({
                        day: dayNo, month, year, today: this.isToday(today, dayNo, month, year),
            ...
            if (this.showWeek) {
                weekNumbers.push(this.getWeekNumber(moment().jYear(week[0].year).jMonth(week[0].month).jDate(week[0].day)));
            }
    ...
    selectDate(dateMeta) {
        let date = moment().jYear(dateMeta.year).jMonth(dateMeta.month).jDate(dateMeta.day);
    ...
    getDaysCountInMonth(month: number, year: number) {
        return 32 - this.daylightSavingAdjust(moment().jYear(year).jMonth(month).jDate(32)).jDate();
    }
    ...
    isDateBetween(start, end, dateMeta) {
        const between = false;
        if (start && end) {
            const date: Moment = moment().jYear(dateMeta.year).jMonth(dateMeta.month).jDate(dateMeta.day);
    ...
    navigateToMonth(prev, groupIndex) {
        if (prev) {
            if (this.numberOfMonths === 1 || (groupIndex === 0)) {
                ...
                const cells = DomHandler.find(prevMonthContainer, '.ui-datepicker-calendar td a');
            ...
            if (this.numberOfMonths === 1 || (groupIndex === this.numberOfMonths - 1)) {
                ...
                const focusCell = DomHandler.findSingle(nextMonthContainer, '.ui-datepicker-calendar td a');
                focusCell.tabIndex = '0';
                focusCell.focus();
            }
        }
    ...
    updateFocus() {
        ...
        if (this.navigationState) {
            if (this.navigationState.button) {
                ...
                if (this.navigationState.backward) {
                    const cells = DomHandler.find(this.contentViewChild.nativeElement, '.ui-datepicker-calendar td a');
    ...
    isValidSelection(value): boolean {
        let isValid = true;
        if (this.isSingleSelection()) {
            if (!this.isSelectable(value.jDate(), value.jMonths(), value.jYear(), false)) {
                isValid = false;
            }
        } else if (value.every(v => this.isSelectable(v.jDate(), v.jMonths(), v.jYear(), false))) {
            if (this.isRangeSelection()) {
    ...
    writeValue(value: any): void {
        this.value = value;
        if (value && this.dataType == 'normaled') {
    ...
    parseDate(value, format) {
            ...
            getNumber = (match) => {
                const isDoubled = lookAhead(match),
                    size = (match === '@' ? 14 : (match === '!' ? 20 :
                        (match.toLowerCase() === 'y'  && isDoubled ? 4 : (match === 'o' ? 3 : 2)))),
                    minSize = (match.toLowerCase() === 'y' ? size : 1),
                    digits = new RegExp('^\\d{' + minSize + ',' + size + '}'),
                    num = value.replaceAll('/','').substring(iValue).match(digits);
                if (!num) {
        ...
        date = this.daylightSavingAdjust(moment().jYear(year).jMonth(month-1).jDate(day));
        if (date.jYear() !== year || date.jMonths() + 1 !== month || date.jDate() !== day) {
...
@NgModule({
    ...
    exports: [PersianCalendarComponent, ButtonModule, SharedModule],
